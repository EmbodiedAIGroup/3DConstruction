...
class RealSensePointCloudAccumulator:
    ...
    def save(self, timestamp):
        ...
        print(f'full_pc after reorder shape: {self.full_pc.shape}')
        # type
        print(f'type(self.full_pc): {type(self.full_pc)}')

        # 转换为Open3D格式
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(self.full_pc.numpy())
        pcd.colors = o3d.utility.Vector3dVector(self.full_pc_colors.numpy())
...
if __name__ == "__main__":
    ...
    accumulator = RealSensePointCloudAccumulator(
        save_dir="../save_results/realsense_scans", # 统一保存到 'realsense_scans' 目录下
        device=device
    )
